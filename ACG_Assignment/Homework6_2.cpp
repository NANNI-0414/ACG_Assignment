...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("B-Spline Surface: ±¤¼±°Ë!");
    ...
    //3. 
    GLuint programID = LoadShaders("VertexShader6_1.txt", "TessControlShader6_2.txt"
        , "TessEvaluationShader6_2.txt", "FragmentShader6_1.txt");
    ...
    GLuint uOuter = glGetUniformLocation(programID, "uOuter");
    glUniform1f(uOuter, 100.0);

    GLuint uInner = glGetUniformLocation(programID, "uInner");
    glUniform1f(uInner, 100.0);

    /*
    float BSplineMatrix[16] = { -1.0 / 6, 3.0 / 6, -3.0 / 6, 1.0 / 6,
                                 3.0 / 6, -6.0 / 6, 3.0 / 6, 0.0 / 6,
                                 -3.0 / 6, 0.0 / 6, 3.0 / 6, 0.0 / 6,
                                 1.0 / 6, 4.0 / 6, 1.0 / 6, 0.0 / 6 };
    GLuint matLoc = glGetUniformLocation(programID, "m");
    glUniformMatrix4fv(matLoc, 1, GL_FALSE, BSplineMatrix);
    */

    float mv0[4] = { -1.0, 3.0, -3.0, 1.0 };
    GLuint mv0Loc = glGetUniformLocation(programID, "mv0");
    glUniform4fv(mv0Loc, 1, mv0);

    float mv1[4] = { 3.0, -6.0, 0.0, 4.0 };
    GLuint mv1Loc = glGetUniformLocation(programID, "mv1");
    glUniform4fv(mv1Loc, 1, mv1);

    float mv2[4] = { -3.0, 3.0, 3.0, 1.0 };
    GLuint mv2Loc = glGetUniformLocation(programID, "mv2");
    glUniform4fv(mv2Loc, 1, mv2);

    float mv3[4] = { 1.0, 0.0, 0.0, 0.0 };
    GLuint mv3Loc = glGetUniformLocation(programID, "mv3");
    glUniform4fv(mv3Loc, 1, mv3);
